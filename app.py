...
class Catalogo:
    ...
    def agregar_reserva(self, solicitante, hotel, tipohabitacion, fecdesde, fechasta, email, imagen_url, telefono):            
        sql = "INSERT INTO reservas (solicitante, hotel, tipohabitacion, fecdesde, fechasta, email, imagen_url, telefono) VALUES (%s, %s, %s, %s, %s, %s, %s, %s )"
        valores = (solicitante, hotel, tipohabitacion, fecdesde, fechasta, email, imagen_url, telefono)
        self.cursor.execute(sql, valores)        
        self.conn.commit()
        return self.cursor.lastrowid
    ...
    def modificar_reserva(self, codigo, nuevo_solicitante, nuevo_hotel, nuevo_tipohabitacion, nuevo_fecdesde, nuevo_fechasta, nuevo_email, nuevo_imagen, nuevo_telefono):
        sql = "UPDATE reservas SET solicitante = %s, hotel = %s, tipohabitacion = %s, fecdesde = %s, fechasta = %s, email = %s,imagen_url = %s, telefono = %s WHERE codigo = %s"
        valores = (nuevo_solicitante, nuevo_hotel, nuevo_tipohabitacion, nuevo_fecdesde, nuevo_fechasta, nuevo_email, nuevo_imagen, nuevo_telefono, codigo)
        self.cursor.execute(sql, valores)
        self.conn.commit()
        return self.cursor.rowcount > 0
    ...
    def mostrar_reserva(self, codigo):
        ...
        if reserva:
            ...
            print(f"Solicitante...: {reserva['solicitante']}")
            print(f"Hotel.........: {reserva['hotel']}")
            print(f"TipoHabitacion: {reserva['tipohabitacion']}")
            print(f"Fecha Desde...: {reserva['fecdesde']}")
            print(f"Fecha Hassta..: {reserva['fechasta']}")
            print(f"Email.........: {reserva['email']}")
            print(f"Imagen........: {reserva['imagen_url']}")
...
#catalogo = Catalogo(host='rojasgabedu.mysql.pythonanywhere-services.com',user='rojasgabedu', password='oliva245', database='rojasgabedu$miapp')
#catalogo = Catalogo(host='localhost', user='root', password='root', database='miapp')
catalogo = Catalogo(host='gabrojas.mysql.pythonanywhere-services.com', user='gabrojas', password='oliva245bd', database='gabrojas$miapp')

# Carpeta para guardar las imagenes.
#RUTA_DESTINO = './static/imagenes/'
RUTA_DESTINO ='/home/gabrojas/mysite/static/imagenes/'
...
def listar_reserva():
    reserva = catalogo.listar_reserva()
...
def agregar_reserva():
    ...
    fecdesde = request.form['fecdesde']
    fechasta = request.form['fechasta']
    ...
    nuevo_codigo = catalogo.agregar_reserva(solicitante, hotel, tipohabitacion, fecdesde, fechasta, email, nombre_imagen, telefono)
    if nuevo_codigo:    
        imagen.save(os.path.join(RUTA_DESTINO, nombre_imagen))
        #Si el producto se agrega con éxito, se devuelve una respuesta JSON con un mensaje de éxito y un código de estado HTTP 201 (Creado).
        return jsonify({"mensaje": "Reserva agregada correctamente.", "codigo": nuevo_codigo, "imagen": nombre_imagen}), 201
    else:
        #Si el producto no se puede agregar, se devuelve una respuesta JSON con un mensaje de error y un código de estado HTTP 500 (Internal Server Error).
...
def modificar_reserva(codigo):
    #Se recuperan los nuevos datos del formulario
    nuevo_solicitante = request.form.get("solicitante")
    nuevo_hotel = request.form.get("hotel")
    nuevo_tipohabitacion = request.form.get("tipohabitacion")
    nuevo_fecdesde = request.form.get("fecdesde")  
    nuevo_fechasta = request.form.get("fechasta") 
    ...
    if 'imagen' in request.files:
        ...
        # Busco el producto guardado
        reserva = catalogo.consultar_reserva(codigo)
        if reserva: # Si existe el producto...
            imagen_vieja = reserva["imagen_url"]
            # Armo la ruta a la imagen
            ruta_imagen = os.path.join(RUTA_DESTINO, imagen_vieja)
            # Y si existe la borro.
            if os.path.exists(ruta_imagen):
        ...
        reserva = catalogo.consultar_reserva(codigo)
        if reserva:
            nombre_imagen = reserva["imagen_url"]

    # Se llama al método modificar_producto pasando el codigo del producto y los nuevos datos.
    if catalogo.modificar_reserva(codigo, nuevo_solicitante, nuevo_hotel, nuevo_tipohabitacion, nuevo_fecdesde, nuevo_fechasta, nuevo_email, nombre_imagen, nuevo_telefono):       
        #Si la actualización es exitosa, se devuelve una respuesta JSON con un mensaje de éxito y un código de estado HTTP 200 (OK).
        return jsonify({"mensaje": "Reserva modificada con Exito.."}), 200
    else:
        #Si el producto no se encuentra (por ejemplo, si no hay ningún producto con el código dado), se devuelve un mensaje de error con un código de estado HTTP 404 (No Encontrado).
        return jsonify({"mensaje": "Reserva Inexistente"}), 403
...
def eliminar_reserva(codigo):
    # Busco el producto en la base de datos
    reserva = catalogo.consultar_reserva(codigo)
    if reserva: # Si el producto existe, verifica si hay una imagen asociada en el servidor.
        imagen_vieja = reserva["imagen_url"]
        # Armo la ruta a la imagen
        ruta_imagen = os.path.join(RUTA_DESTINO, imagen_vieja)
        # Y si existe, la elimina del sistema de archivos.
        ...
        if catalogo.eliminar_reserva(codigo):
            #Si el producto se elimina correctamente, se devuelve una respuesta JSON con un mensaje de éxito y un código de estado HTTP 200 (OK).
            return jsonify({"mensaje": "Reserva eliminada con Exito.."}), 200
        else:
            #Si ocurre un error durante la eliminación (por ejemplo, si el producto no se puede eliminar de la base de datos por alguna razón), se devuelve un mensaje de error con un código de estado HTTP 500 (Error Interno del Servidor).
            return jsonify({"mensaje": "ERROR al eliminar la Reserva"}), 500
    else:
        #Si el producto no se encuentra (por ejemplo, si no existe un producto con el codigo proporcionado), se devuelve un mensaje de error con un código de estado HTTP 404 (No Encontrado). 
        return jsonify({"mensaje": "Reserva Inexistente"}), 404
